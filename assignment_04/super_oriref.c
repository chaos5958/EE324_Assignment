...
#define ACCEPT_NUM (LISTEN_QUEUE_NUM)  
...
void add_client(int connfd, pool *p) 
{
	...
	if (p->clientfd[i].fd < 0) { 
	    ...
	    p->clientfd[i].fd = connfd;                 //line:conc:echoservers:beginaddclient
...
void check_clients(pool *p) 
{
	...
	if ((connfd > 0) && (FD_ISSET(connfd, &p->ready_set))) { 
        ...
        byte_cnt = 0;
        if((n = Rio_readnb(&rio, buf + byte_cnt, MAXLINE)) != 0)
        {
            byte_cnt += n;
            if(p->clientfd[i].is_hdr)
            {
                hdr = (kaza_hdr_t *)buf;
		...
		p->clientfd[i].fd = -1;          //line:conc:echoservers:endremove
        p->clientfd[i].is_hdr =false;
	}
    }
}
